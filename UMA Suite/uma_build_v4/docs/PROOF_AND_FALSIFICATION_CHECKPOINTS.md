...
| P3 | The gradient stress T^{(∇M)}_{μν} contracted with any null vector is non-negative (NEC compliance) | `uma/rsls/memory.py: gradient_stress, nec_violation` | Contract a **genuinely null** k in (2+1) Minkowski; if min < 0, NEC violated. Until 2026-08-19 this gate could not fail: it discarded the contraction and returned min((k·∇M)²), a square. Now falsifiable — see `tests/test_rsls.py::TestNECIsAGateThatCanFail`, which includes the control showing a non-null contraction goes negative. |
...
| N2 | Stage 5 cone aperture in saturation layer with frame-dragging | `tests/test_frame_dragging.py::test_cone_aperture_open_with_drag` | saturation margin = **+0.077** | **NOT FALSIFIABLE AS WRITTEN.** aperture = 2·sqrt(c²+G) with G = (R ∂_R β^φ)² ≥ 0, so margin ≥ 0 identically. In Stage 5 it is also computed once from a static prescribed profile before the time loop, so it carries no dynamical information. |
| N3 | Stage 5 cone aperture without frame-dragging (control) | `tests/test_frame_dragging.py::test_cone_aperture_closed_without_drag` | margin = **0.0** (at the c_diff floor) | This one *is* informative: it is the G = 0 case of the identity above, so it pins the floor. |
| N4 | Stage 5 Lyapunov with frame-dragging | `examples/rsls_frame_dragging.py` | **WITHDRAWN** (was +1.127) | The statistic is a discretisation artifact: λ·dt is constant under refinement. The estimator now refuses. See `studies/rsls_lyapunov/README.md`. |
| N5 | Stage 5 Lyapunov without frame-dragging (control) | `examples/rsls_frame_dragging.py` | λ = **−0.037448**, converged | The one converged exponent in the family: the undragged kernel is measurably not chaotic over this window. Caveat: 39.1% of steps are non-differentiable, and it is a finite-time estimate over a transient, not a property of a limiting invariant measure. |
...
| N7 | Stage 6 cone stays strictly positive throughout coupling | `tests/test_stage6.py::test_cone_strictly_positive_throughout` | True | **NOT FALSIFIABLE AS WRITTEN** — the same 2·sqrt(c²+G) identity as N2. Stage 6 does recompute it from the evolving β^φ, so it is at least dynamical, but > 0 remains automatic. |
| N8 | Stage 6 Lyapunov under coupling | `tests/test_stage6.py::TestStage6Lyapunov::test_lyapunov_is_refused_not_reported` | **REFUSED** (was +19.4) | The cited test `test_positive_lyapunov` no longer exists; it was replaced when the claim was withdrawn. The gate now asserts the estimator refuses and that `lyapunov_max` stays NaN, so no caller can read an unconverged statistic as a result. |
