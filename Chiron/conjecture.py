...
TWIN NOTE: this is the Chiron spine copy of the seed module
Primus/src/primus/conjecture.py. The exact core (parser, evaluator,
verifier) is kept line-identical; only stage 0 (chiron.collapse instead
of the seed engine), _GATE_ENGINE_FIRST, and the usage strings differ.
Port changes to BOTH copies, the way the two collapse engines are
twinned. Capability honesty: chiron's polynomial family is uncapped and
an everywhere-defined integer-valued rational function is always a
polynomial, so with the add/sub/mul/div function set this layer adds
contract parity to the spine, not new stamping reach — the seed engine
(degree-6 poly cap) is where it adds capability today.

The engine (`chiron.collapse`) searches an explicit, finite hypothesis
...
Usage:
    from conjecture import conjecture, verify_closed_form
    cert = conjecture([0, 2, 130, 2190, 16388, ...])
    cert["status"], cert.get("expression")

    $ python3 conjecture.py "1 2 4 8 16 32 64 128"
    $ python3 conjecture.py selftest
"""
...
SCHEMA = "primus.conjecture/1"   # same certificate contract, chiron spine

# Exact-arithmetic work bounds. Same values as the seed copy and as
# primus.certify's adversarial bounds (documented in Primus/SCHEMA.md).
...
def conjecture(terms: Sequence[int], *, seed: int = 0, population: int = 1500,
    ...
    Stage 0: `chiron.collapse` on the full input — if the engine
...
def render(cert: Dict[str, Any]) -> str:
    lines = ["CHIRON CONJECTURE  (guess-and-prove: stochastic proposer, exact stamp)"]
...
_GATE_ENGINE_FIRST = False   # chiron subsumes GP reach; gates prove the pipeline
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if not ints:
        print("usage: python3 conjecture.py \"1 2 4 8 16 ...\" | selftest")
        return 2
