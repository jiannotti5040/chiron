# Chiron — Public Grow (open `crescere`)

This is the **open, community-contributable Congress.** It runs on the same engine
...
grow-public/
  ...
  chiron_memory.json     the growing public Congress (committed; PR-reviewed)
...
It pulls the current public Congress, ingests, and (if `git.push` is on) commits
**both the Congress and the crawl cursor** — saving locally *and* pushing.

## Regulatory & governmental law

A ready profile opens public legal corpora (constitutions, rights instruments,
statutes) into the Congress under a `regulation` domain — the material JDICert
