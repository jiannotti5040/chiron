...
3. feeds each item through `assimilate` under its **subject** — which spawns or
   ...
   grows the Congress; linked items become a self-extending **frontier**, so it keeps
   discovering,
4. **grows cross-domain concepts** from what it has now proven,
5. **pushes** the grown Congress back when it fills (saving **locally and** to
   GitHub), and
6. **resumes** where it left off — three ways over: a local cursor, that cursor
   committed to git, and the Congress's own `ingested_sources` ledger.
...
- **Resume is robust.** The crawl cursor (`chiron_grow_state.json`) travels with the
  repo, and the Congress carries its own ledger of everything eaten — so a fresh
