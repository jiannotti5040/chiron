...
everything — that is gate 26), and the full engine's source does not ship
...
- **Same contract as the full engine:** exact integer/Fraction arithmetic
  ...
  here too.
- **Not the full engine, not derived from vault source, strictly weaker:**
  ...
  recurrences counting coefficients *and* seeds). The full engine stamps
...
The full engine's real outputs on external data are graded in
